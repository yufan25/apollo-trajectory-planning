...
std::vector<Condition> EndConditionSampler::SampleLatEndConditions() const {
  ...
  std::array<double, 3> end_d_candidates = {0.1, -2.0, 2.0};
  std::array<double, 4> end_s_candidates = {5.0, 10.0, 20.0, 40.0};
...
//end_s_conditions存储了每个时间点所有可能的速度取值（每个时间点6个速度选取，一共8+1个时间点,头尾只有一个速度取值，中间每个点6个，一共50个)
