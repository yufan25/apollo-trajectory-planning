...
csc* csc_spalloc(c_int m, c_int n, c_int nzmax, c_int values, c_int triplet) {
  csc *A = (csc *) csc_calloc(1, sizeof(csc)); /* allocate the csc struct */
  ...
  A->p     = (c_int *) csc_malloc(triplet ? nzmax : n + 1, sizeof(c_int));
  A->i     = (c_int *) csc_malloc(nzmax,  sizeof(c_int));
  A->x     = values ? (c_float *) csc_malloc(nzmax,  sizeof(c_float)) : OSQP_NULL;
...
csc* triplet_to_csc(const csc *T, c_int *TtoC) {
  ...
  w  = (c_int *) csc_calloc(n, sizeof(c_int));                  /* get workspace */
...
csc* triplet_to_csr(const csc *T, c_int *TtoC) {
  ...
  nz = T->nz;
  C  = csc_spalloc(m, n, nz, Tx != OSQP_NULL, 0);     /* allocate result */
  w  = (c_int *) csc_calloc(m, sizeof(c_int));                  /* get workspace */
...
c_int* csc_pinv(c_int const *p, c_int n) {
  ...
  pinv = (c_int *) csc_malloc(n, sizeof(c_int));     /* allocate result */
