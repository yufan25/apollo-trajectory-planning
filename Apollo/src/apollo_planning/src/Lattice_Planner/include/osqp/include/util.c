...
OSQPSettings* copy_settings(const OSQPSettings *settings) {
 // OSQPSettings *new; 
  OSQPSettings *new1 = (OSQPSettings *) malloc(sizeof(OSQPSettings));

  if (!new1) return OSQP_NULL;

  // Copy settings
  // NB. Copying them explicitly because memcpy is not
  // defined when PRINTING is disabled (appears in string.h)
  new1->rho = settings->rho;
  new1->sigma = settings->sigma;
  new1->scaling = settings->scaling;

# if EMBEDDED != 1
  new1->adaptive_rho = settings->adaptive_rho;
  new1->adaptive_rho_interval = settings->adaptive_rho_interval;
  new1->adaptive_rho_tolerance = settings->adaptive_rho_tolerance;
# ifdef PROFILING
  new1->adaptive_rho_fraction = settings->adaptive_rho_fraction;
# endif
# endif // EMBEDDED != 1
  new1->max_iter = settings->max_iter;
  new1->eps_abs = settings->eps_abs;
  new1->eps_rel = settings->eps_rel;
  new1->eps_prim_inf = settings->eps_prim_inf;
  new1->eps_dual_inf = settings->eps_dual_inf;
  new1->alpha = settings->alpha;
  new1->linsys_solver = settings->linsys_solver;
  new1->delta = settings->delta;
  new1->polish = settings->polish;
  new1->polish_refine_iter = settings->polish_refine_iter;
  new1->verbose = settings->verbose;
  new1->scaled_termination = settings->scaled_termination;
  new1->check_termination = settings->check_termination;
  new1->warm_start = settings->warm_start;
# ifdef PROFILING
  new1->time_limit = settings->time_limit;
# endif
